...
This repo uses a **feature-branch + no-fast-forward merge** workflow for all changes. Every change — documentation, examples, playbook edits, skill tweaks, even small fixes — goes through a branch and gets merged into `main` with an explicit merge commit. The pattern matches the convention in [`ds-case-interview-skill`](https://github.com/ruoyanhuang216/ds-case-interview-skill) (this skill's parent reference repo).
...
- **Aligns with the sibling repo** ([`ds-case-interview-skill`](https://github.com/ruoyanhuang216/ds-case-interview-skill)) so muscle memory transfers.
...
1. **Branch named `notes/<topic>`** (mirrors the `ds-case-interview-skill` convention).
